...
select avg (cast (Sale_Price as money)) as average_Sale_Price
FROM [pc_data].[dbo].[pc_data];

-- 6. Find the highest and lowest Sale Price.
select max (cast (Sale_Price as int)) as highest_Sale_Price ,
		min (cast (Sale_Price as int)) as lowest_sale_price
FROM [pc_data].[dbo].[pc_data];
...
select sum (cast (Sale_Price as int)) as total_revenue
FROM [pc_data].[dbo].[pc_data];

-- 12. Calculate total profit (SUM of Sale Price - Cost Price). (ERROR)
select sum (cast (Sale_Price - Cost_Price as int)) as total_profit
FROM [pc_data].[dbo].[pc_data]

-- 13. Find the average Discount Amount.
select avg (cast (Discount_Amount as int)) as average_Discount_Amount
FROM [pc_data].[dbo].[pc_data];
...
select PC_Make,
		sum (cast (Sale_Price as int)) as revenue_per_pc_make
FROM [pc_data].[dbo].[pc_data]
group by PC_Make;

-- 16. Find average Sale Price per Storage Type.
select Storage_Type,
		sum (cast (Sale_Price as int)) as revenue_per_Storage_Type
FROM [pc_data].[dbo].[pc_data]
group by Storage_Type;

-- 17. Calculate total revenue per Shop Name.
select Shop_Name,
		sum (cast (Sale_Price as int)) as revenue_per_Shop_Name
FROM [pc_data].[dbo].[pc_data]
group by Shop_Name;

-- 18. Calculate total revenue per Sales Person Name.
select Sales_Person_Name,
		sum (cast (Sale_Price as int)) as revenue_per_Sales_Person_Name
FROM [pc_data].[dbo].[pc_data]
group by Sales_Person_Name;

-- 19. Find average Credit Score per Payment Method.
select Payment_Method, 
		avg (cast (Credit_Score as int)) as average_Credit_Score_per_Payment_Method
FROM [pc_data].[dbo].[pc_data]
group by Payment_Method;
...
select Shop_Name,
	sum (cast (Sale_Price - Cost_Price as int)) as total_profit_per_shop_name
FROM [pc_data].[dbo].[pc_data]
group by Shop_Name;
...
FROM [pc_data].[dbo].[pc_data]
group by Continent
order by sum ( cast (Sale_Price as int)) DESC;

-- 24. Calculate average Sale Price per RAM size.
select RAM, 
		avg (cast (Sale_Price as int)) as average__PER_RAM
FROM [pc_data].[dbo].[pc_data]
group by RAM;

-- 25. Find the PC Model with the highest Sale Price.
select top 1 PC_Model,
			cast (Sale_price as int) as PC_Model_with_the_highest_Sale_Price
FROM [pc_data].[dbo].[pc_data]
order by cast (Sale_price as int) DESC;

-- 26. Calculate the average number of days between Purchase Date and Ship Date
select AVG(DATEDIFF(DAY, 
try_cast (Purchase_Date as datetime2),
try_cast (Ship_Date as datetime2))) as average_number_of_days_between_Purchase_and_Ship_Date
FROM [pc_data].[dbo].[pc_data]
...
select Storage_Capacity,
		sum (cast (Sale_Price as int)) as revenue_per_Storage_Capacity
FROM [pc_data].[dbo].[pc_data]
group by Storage_Capacity;

-- 29. Identify sales where Sale Price is lower than PC Market Price.
select cast (Sale_Price as int),
		cast (PC_Market_Price as int)
FROM [pc_data].[dbo].[pc_data]
where cast (Sale_Price as int) < cast (PC_Market_Price as int);

-- 30. Rank Sales Person Name by Total Sales per Employee using a window function.
select Sales_Person_Name,
    sum (cast(Sale_Price as int)) as Total_Sales,
	rank() over (order by sum(cast(Sale_Price as int)) DESC) as Rank_number
-- row_number() over (order by sum(cast(Sale_Price as int)) DESC) as row_numbers (if we have to look for row numbers)
FROM [pc_data].[dbo].[pc_data]
group by Sales_Person_Name;
---------------------------------------------------------------------------------------------------
select * 
